...
/** Fallback: build full ResumeData from LaTeX when Claude omits the RESUME_DATA tag.
 *  Parses name/email/phone, summary, skills, experience entries (title/company/dates/bullets),
 *  education entries, and certifications — so Edit Resume is fully populated, not just partial. */
function buildResumeDataFromLatex(latex: string): Record<string, unknown> {

  // ── Strip common LaTeX markup from a string ───────────────────────────────
  const stripTex = (s: string): string =>
    s
      .replace(/\\textbf\{([^}]*)\}/g, "$1")
      .replace(/\\textit\{([^}]*)\}/g, "$1")
      .replace(/\\emph\{([^}]*)\}/g, "$1")
      .replace(/\\href\{[^}]*\}\{([^}]*)\}/g, "$1")
      .replace(/\\(?:rupee|texteuro|textsterling|textbackslash)\{?\}?/g, "")
      .replace(/\\[a-zA-Z]+\*?\{([^}]*)\}/g, "$1")  // \cmd{text} → text
      .replace(/\\[a-zA-Z@]+\*?/g, "")               // bare \cmd → gone
      .replace(/[{}$]/g, "")
      .replace(/\s{2,}/g, " ")
      .trim();

  // ── Extract a named section's raw content ────────────────────────────────
  // Stops at the next \section*{…} or \end{document}
  ...
  // ── Personal info ─────────────────────────────────────────────────────────
  // Name: look for the large/bold header block generated by buildLatexFromData
  // and also Claude's common alternative patterns
  const nameMatch =
    latex.match(/\{\\Large\s*\\textbf\{([^}]+)\}\}/i) ||
    latex.match(/\\Large\s*\\textbf\{([^}]+)\}/i)     ||
    latex.match(/\\huge\s*\\textbf\{([^}]+)\}/i)      ||
    latex.match(/\\begin\{center\}[\s\S]{0,300}?\\textbf\{([^}]{3,60})\}/i);
  const name = nameMatch ? stripTex(nameMatch[1]) : "";
  ...
  const summaryRaw = getSection("Professional Summary");
  const summary = stripTex(summaryRaw.replace(/\\vspace\{[^}]*\}/g, ""));

  // ── Skills ────────────────────────────────────────────────────────────────
  // buildLatexFromData emits skills as a single comma-separated line
  const skillsRaw = getSection("Skills");
  const skills = stripTex(skillsRaw)
    .split(",")
    .map(s => s.trim())
    .filter(Boolean);

  // ── Certifications ────────────────────────────────────────────────────────
  const certsRaw = getSection("Certifications");
  const certMatches = [...certsRaw.matchAll(/\\item\s+([\s\S]*?)(?=\\item\b|\\end\{itemize\}|$)/g)];
  const certifications = certMatches.map(m => stripTex(m[1])).filter(Boolean);

  // ── Experience ────────────────────────────────────────────────────────────
  // buildLatexFromData pattern per entry:
  //   \noindent\textbf{Title} \hfill StartDate -- EndDate\\
  //   \textit{Company, Location}
  //   \begin{itemize}...\item bullet...\end{itemize}
  //
  // Strategy: find every "header" (textbf + \hfill + dates + \\ + textit company),
  // record their positions, then slice out the body between consecutive headers.
  const expRaw = getSection("Work Experience");

  // Match the per-entry header line: \textbf{title} \hfill dates\\ \textit{company}
  const expHeaderRe = /\\textbf\{([^}]+)\}\s*\\hfill\s*(.*?)\\\\\s*\\textit\{([^}]+)\}/g;
  interface ExpHeader { idx: number; endIdx: number; title: string; dates: string; companyLoc: string; }
  const expHeaders: ExpHeader[] = [];
  let hm: RegExpExecArray | null;
  while ((hm = expHeaderRe.exec(expRaw)) !== null) {
    expHeaders.push({ idx: hm.index, endIdx: hm.index + hm[0].length, title: hm[1], dates: hm[2], companyLoc: hm[3] });
  }

  const experience = expHeaders.map((h, i) => {
    // Body = everything between this header's end and the next header's start
    const bodyEnd = i + 1 < expHeaders.length ? expHeaders[i + 1].idx : expRaw.length;
    const body = expRaw.slice(h.endIdx, bodyEnd);

    // Dates: "Jan 2020 -- Present"  or just "Jan 2020"
    const dateParts = h.dates.split(/\s*--\s*/);
    const startDate = stripTex(dateParts[0] || "").trim();
    const endDate   = stripTex(dateParts[1] || "").trim();

    // Company + location: "Acme Corp, New York"
    const cParts  = h.companyLoc.split(",");
    const company  = stripTex(cParts[0] || "").trim();
    const location = stripTex(cParts.slice(1).join(",") || "").trim();

    // Bullets: each \item ... up to next \item or \end{itemize}
    const bulletRe = /\\item\s+([\s\S]*?)(?=\\item\b|\\end\{itemize\}|$)/g;
    const bullets: string[] = [];
    let bm: RegExpExecArray | null;
    while ((bm = bulletRe.exec(body)) !== null) {
      const b = stripTex(bm[1]);
      if (b) bullets.push(b);
    }

    return {
      id: `exp-${i + 1}`,
      title: stripTex(h.title),
      company,
      location,
      startDate,
      endDate,
  ...
  //   Institution
  const eduRaw = getSection("Education");

  const eduHeaderRe = /\\textbf\{([^}]+)\}\s*\\hfill\s*(.*?)\\\\/g;
  interface EduHeader { idx: number; endIdx: number; degree: string; year: string; }
  const eduHeaders: EduHeader[] = [];
  let em: RegExpExecArray | null;
  while ((em = eduHeaderRe.exec(eduRaw)) !== null) {
    eduHeaders.push({ idx: em.index, endIdx: em.index + em[0].length, degree: em[1], year: em[2] });
  }

  const education = eduHeaders.map((h, i) => {
    // Institution is the text immediately after the header line, before the next header / vspace
    const bodyEnd = i + 1 < eduHeaders.length ? eduHeaders[i + 1].idx : eduRaw.length;
    const body = eduRaw.slice(h.endIdx, bodyEnd);
    // First non-empty non-command line is the institution
    const institution = stripTex(
      body.split("\n").map(l => l.trim()).find(l => l && !l.startsWith("\\")) || ""
    );
    return {
      id: `edu-${i + 1}`,
      degree:      stripTex(h.degree),
      institution,
      year:        stripTex(h.year).trim(),
    };
  });
